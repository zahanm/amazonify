...
is item-based collaborative filtering, which aims to utilize
...
\subsection{Matrix factorization}
This particular model not suited to being run on the smaller and sparser sets.
We tried to run it on the Yelp and original Amazon datasets, but the stochastic
gradient minimization was unable to find convergance in a tenable number of
iterations. As a result, we concentrated on the high-activity Amazon dataset.

\subsubsection{Amazon high-activity dataset}
It ran very well, however, on this dataset and the results are shown in
Table~\ref{table:modelthree_subset} and
Figure~\ref{fig:modelone_subset}. We varied the number of latent dimensions ($f$)
being learned over in the common user-product space and to take into account
overfitting effects. Just as we suspected, even though reducing the number of
latent dimensions raised the RMSE on the training set, it reduced the RMSE on
the test set. Apparently, teh training set was being overfit at $f = 20$ and
$f = 10$ was able to capture just enough variance.
...
It is worth noting, that we also tried to run this model with the regularization
parameter ($\lambda$) mentioned in matrix factorization papers \cite{bib:matrixfact}.
It`s purpose is to avoid overfitting effects by penalizing extremely large values of 
any particular coefficient corresponding to a dimension in the new space.
It turns out for our purposes though, it ended up outweighing the gradient descent
cost function being minimized even for absurdly small values of $\lambda$.
As a result, in the final version of our factorization method implementation,
we ended up excluding the regularization parameter.
